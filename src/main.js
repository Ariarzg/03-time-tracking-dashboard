  ...
  categories.forEach((category) => {
    ...
    html += `
      <section class="bg-${titleFormatted} rounded-2xl overflow-hidden relative">
        ...
        />
        <div
          class="p-6 mt-10 h-[7.5rem] sm:h-[11.5rem] bg-navy-900 hover:bg-navy-900/95 cursor-pointer rounded-2xl grid grid-cols-1 relative z-20"
        >
          ...
          <div class="flex items-center sm:items-start sm:flex-col mt-2 sm:mt-4 mb-4">
            <p class="text-3xl sm:text-[3.25rem] font-light">${currentHours}hrs</p>
            <p class="text-navy-200 font-light text-end ml-auto sm:ml-0">Last ${timePeriodFormatted} - ${previousHours}hrs</p>
          </div>
  ...
  return `
    <section class="bg-navy-900 rounded-2xl overflow-hidden sm:row-span-2 grid grid-cols-3 sm:grid-cols-1">
      <div class="bg-accent rounded-2xl w-full col-span-3 sm:col-span-1">
        <div class="flex px-6 py-7 sm:pb-18 gap-4 sm:gap-10 items-center sm:flex-col sm:items-start">
          <img
          ...
          <div class="">
            ...
            <h2 class="text-white text-2xl sm:text-4xl font-light">Jeremy Robson</h2>
          </div>
        </div>
      </div>
      <a class="inline text-center sm:text-start sm:h-min p-4 sm:px-6 sm:py-2 sm:mt-4 ${
        timePeriod === 'daily' ? '' : 'text-navy-500'
      } cursor-pointer hover:text-white" id="daily">Daily</a>
      <a class="inline text-center sm:text-start sm:h-min p-4 sm:px-6 sm:py-2 ${
        timePeriod === 'weekly' ? '' : 'text-navy-500'
      } cursor-pointer  hover:text-white" id="weekly">Weekly</a>
      <a class="inline text-center sm:text-start sm:h-min p-4 sm:px-6 sm:py-2 sm:mb-4 ${
